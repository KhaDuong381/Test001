...
{
    public partial class 柱リストを入力 : Window
    {
        private Document doc;
        private ProjectData _projectData;
        public 柱リストを入力(Document doc)
        {
            InitializeComponent();
            this.doc = doc;
        ...
        public void Load()
        {
            _projectData.SyncHashiraWithNameKai(); // Đồng bộ danh sách tầng
            DataContext = _projectData.リスト;
        }

        private void AddButton_Click(object sender, RoutedEventArgs e)
        {
            if (FloorNameListBox.SelectedItem is 柱リスト selectedFloor)
            {
                ...
                while (!validName)
                {
                    ...
                    var inputDialog = new InputDialog("柱の名前を入力してください:");
                    ...
                    {
                        MessageBox.Show("柱の名前を入力してください。");
                        continue;
                    }

                    // Kiểm tra tên trùng lặp
                    if (selectedFloor.柱.Any(c => c.Name == columnName))
                    {
                ...
                selectedFloor.柱.Add(new 柱 { Name = columnName });
                ...
                ColumnListBox.ItemsSource = selectedFloor.柱;

                // Thông báo thành công
                MessageBox.Show($"柱 '{columnName}' を階 '{selectedFloor.各階}' に追加し、保存しました。");
            }
            else
            {
        ...
        private void EditButton_Click(object sender, RoutedEventArgs e)
        {
            if (FloorNameListBox.SelectedItem is 柱リスト selectedFloor)
            {
                if (ColumnListBox.SelectedItem is 柱 selectedColumn)
                {
                    // Mở cửa sổ chỉnh sửa thông tin cột
                    var editWindow = new 柱の配置(doc, selectedColumn.柱の配置, _projectData) { Left = 500, Top = 200 };
                    if (editWindow.ShowDialog() == true)
                    {
                        // Đảm bảo cập nhật lại instance đã chỉnh sửa
                        selectedColumn.柱の配置 = editWindow.DataContext as Z柱の配置;
                        ...
                        MessageBox.Show($"柱 '{selectedColumn.Name}' を更新し、保存しました。");
                    }
                }
                else
                {
                    MessageBox.Show("編集する柱を選択してください。");
                }
            }
            else
            {
        ...
        private void DeleteButton_Click(object sender, RoutedEventArgs e)
        {
            if (FloorNameListBox.SelectedItem is 柱リスト selectedFloor)
            {
                if (ColumnListBox.SelectedItem is 柱 selectedColumn)
                {
                    ...
                    var result = MessageBox.Show($"柱 '{selectedColumn.Name}' を階 '{selectedFloor.各階}' から削除しますか？",
                                                 "削除の確認", MessageBoxButton.OK, MessageBoxImage.Warning);

                    if (result == MessageBoxResult.OK)
                    {
                        // Xóa cột khỏi danh sách
                        selectedFloor.柱.Remove(selectedColumn);

                        // Lưu dữ liệu
                        StorageUtils.SaveProject(doc, _projectData);

                        // Thông báo xóa thành công
                        MessageBox.Show($"柱 '{selectedColumn.Name}' を削除し、保存しました。");
                    }
                }
                else
                {
                    MessageBox.Show("削除する柱を選択してください。");
                }
            }
            else
            {
            ...
            if (FloorNameListBox.SelectedItem is 柱リスト selectedFloor)
            {
                if (ColumnListBox.SelectedItem is 柱 selectedColumn)
                {
                    ...
                    while (!validName)
                    {
                        ...
                        var inputDialog = new InputDialog("新しい柱の名前を入力してください:");
                        ...
                        if (newName != selectedColumn.Name && selectedFloor.柱.Any(c => c.Name == newName))
                        {
                    ...
                    MessageBox.Show($"柱の名前を '{newName}' に更新し、保存しました。");
                }
                else
                {
                    MessageBox.Show("名前を編集する柱を選択してください。");
                }
            }
            else
            {
            ...
            if (FloorNameListBox.SelectedItem is 柱リスト selectedFloor)
            {
                if (ColumnListBox.SelectedItem is 柱 selectedColumn)
                {
                    ...
                    while (!validName)
                    {
                        ...
                        var inputDialog = new InputDialog("コピーする柱の新しい名前を入力してください:");
                        if (inputDialog.ShowDialog() != true)
                        {
                            return; // Người dùng hủy thao tác
                        }
                        ...
                        if (selectedFloor.柱.Any(c => c.Name == newColumnName))
                        {
                    ...
                    {
                        Name = newColumnName,
                        柱の配置 = selectedColumn.柱の配置
                    };

                    //Thêm cột mới vào danh sách
                    selectedFloor.柱.Add(newColumn);
                    ...
                    MessageBox.Show($"柱 '{selectedColumn.Name}' を '{newColumnName}' としてコピーし、保存しました。");
                }
                else
                {
                    MessageBox.Show("コピーする柱を選択してください。");
                }
            }
            else
            {
